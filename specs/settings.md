...
- **As a user**, my language-to-voice mappings are saved to `voice_mapping.json` and restored on startup, so each language routes to the correct voice.
...
| `voice_mapping.json` | VoiceMapping (language_voice map, fallback_voice_key) |
...
- `src-tauri/src/tts/voice_mapping.rs` — language routing config persistence

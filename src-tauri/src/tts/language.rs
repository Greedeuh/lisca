...
mod tests {
    ...
    #[test]
    fn detects_english() {
        assert_eq!(detect_language_family("Hello, world! This is a test."), Some("en"));
    }

    #[test]
    fn detects_french() {
        assert_eq!(detect_language_family("Bonjour, comment allez-vous aujourd'hui?"), Some("fr"));
    }

    #[test]
    fn detects_german() {
        assert_eq!(detect_language_family("Guten Tag, wie geht es Ihnen?"), Some("de"));
    }

    #[test]
    fn detects_spanish() {
        assert_eq!(detect_language_family("Hola, ¿cómo estás? Espero que bien."), Some("es"));
    }

    #[test]
    fn detects_italian() {
        assert_eq!(detect_language_family("Buongiorno, come stai oggi?"), Some("it"));
    }

    #[test]
    fn detects_russian() {
        assert_eq!(detect_language_family("Привет, как дела? Сегодня хорошая погода."), Some("ru"));
    }
